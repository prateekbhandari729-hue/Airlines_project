...
/* ---------------------------------------------------------
   1b. Mobile menu — hamburger toggle for the nav bar
--------------------------------------------------------- */
function initMobileMenu() {
  ...
  // Close the menu once a destination is chosen (mobile UX)
  nav.querySelectorAll('.nav-item').forEach((item) => {
    item.addEventListener('click', closeMenu);
  });

  // Close when tapping/clicking outside the open menu
  ...
  // If the viewport is resized back to desktop while the menu is open, reset it
...
function initSearchTabs() {
  ...
  const departureInput = dateField.querySelector('input');
    ...
    tab.addEventListener('click', () => {
      ...
      if (tab.textContent.trim() === 'Round Trip') {
        if (!returnField) {
          ...
          // Keep the return date in sync with the chosen departure date
        ...
        // Switch the grid to the 6-column layout sized for the return field
        searchRow.classList.add('has-return');
      } else {
        if (returnField) returnField.remove();
        searchRow.classList.remove('has-return');
      }
    });
  });
}
...
function initFlightSearch() {
  ...
  searchBtn.addEventListener('click', () => {
    ...
    const returnInput = document.querySelector('.field-return input');
    ...
    if (returnInput && !returnInput.value) {
      shakeElement(searchBtn);
      showToast('Please select a return date.');
      return;
    }

    if (returnInput && returnInput.value < date.value) {
      shakeElement(searchBtn);
      showToast('Return date can\'t be before the departure date.');
      return;
    }

    const tripSummary = returnInput
      ? `${from.value} → ${to.value}, depart ${formatDate(date.value)} · return ${formatDate(returnInput.value)}`
      : `${from.value} → ${to.value} on ${formatDate(date.value)}`;

    showToast(`Searching flights: ${tripSummary} for ${passengers.value}…`);
